...
import { useWordReplaceStore } from './word_replace'
...
export const useSpeechStore = defineStore('speech', {
    ...
    actions: {
        ...
        toggle_listen() {
            ...
            if (defaultStore.speech.listening) {
                ...
                defaultStore.speech.onend = () => {
                    ...
                    if (defaultStore.speech.listening) defaultStore.speech.start()
                }
                defaultStore.speech.onerror = (event: any) => {
                    let desc = ''
                    if (event.error === 'no-speech') return // web-speech: no sound detected
                    if (event.error === 'not-allowed') desc = i18n.t('alerts.mic_error')
                    if (event.error === 'aborted') desc = i18n.t('alerts.device_in_use')
        ...
        async on_submit(log: any, index: number) {
            ...
            // new line delay
            if (logStore.wait_interval) clearTimeout(logStore.wait_interval)
            ...
            if (log.isFinal) {
                ...
                // text-to-speech
                if (this.tts.enabled && this.tts.voice) this.speak(log.transcript)

                // fadeout text
                if (text.enable_fade)
                    setTimeout(() => {
                        if(!logStore.logs[i].pause) return
                        ...
                        while (i >= 0 && pauses < 2) {
                            ...
                            if (logStore.logs[i].pause) pauses += 1
                            i -= 1
                        }
